...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = dompurify.sanitize('<div class="search__result">No results for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 38.72;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -9.14;
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // when someone clicks on the marker, show details
...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var page = window.location.pathname; // selects the current URL
    ...
    } else if (!isHearted && page === "/hearts") {
      // checks if the current path is the hearts route
      _this.heart.classList.remove('heart__button--hearted'); // removes the hearted class
      _this.parentNode.parentNode.parentNode.parentNode.remove(); // removes the main parent element of .store
    }
  }).catch(console.error);
}
